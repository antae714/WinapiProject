...
void MoveMap2::Set(string p_src, string p_dest, int p_x, int p_y)
{
	m_src = p_src;
	m_dest = p_dest;
	m_pos_x = p_x;
...
void MoveMap2::KeyInput()
{
	...
	if (tempcom3->getpivot().gety() < 0) {
		direction = -1;
	}

	LevelData::DeleteLevel("text");
	LevelData::LevelLode(dest);
	allObject->addPivotGroup(enumObj::conversion(dest), Vector2(0, 720 * direction));
	...
	for (int i = 0; i < 24; i++)
	{
		allObject->addPivotGroup(enumObj::conversion(src), Vector2(0, -30 * direction));
		allObject->addPivotGroup(enumObj::conversion(dest), Vector2(0, -30 * direction));
		camera->Render();
		gameTime->TimeUpdate();
	}
